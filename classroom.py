...
return jsonify({'tx_id': tid, 'to': to_addr, 'amount': amount, 'new_balance': new_balance}), 200
...
return jsonify({'tx_id': tid, 'from': from_addr, 'to': to_addr, 'amount': amount}), 200
...
def route_balances():
db = get_db()
rows = db.execute('SELECT name, public_address, balance FROM users ORDER BY balance DESC').fetchall()
...
db = get_db()
rows = db.execute('SELECT name, public_address, balance FROM users ORDER BY balance DESC LIMIT 10').fetchall()
out = [{'name': r['name'], 'address': r['public_address'], 'balance': r['balance']} for r in rows]
...
def route_ledger():
db = get_db()
rows = db.execute('SELECT timestamp, from_addr, to_addr, amount, note FROM transactions ORDER BY timestamp DESC LIMIT 200').fetchall()
